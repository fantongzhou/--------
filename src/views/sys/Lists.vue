      ...
      <template>
        <el-container>
          ...
          <el-main class="elMain">
              ...
              <el-table-column align="center" label="操作">
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    type="danger"
                    @click="removeSingerByName(scope.row.listid,scope.row.songname)"
                    >删除</el-button
                  >
                </template>
              </el-table-column>
            ...
            >
              <el-form :model="form">
                <el-form-item label="listid" :label-width="formLabelWidth">
                  <el-input
                    v-model="form.listid"
                    autocomplete="off"
                  ></el-input>
                ...
                <el-form-item label="歌曲地址" :label-width="formLabelWidth">
                  <el-input v-model="form.songurl" autocomplete="off"></el-input>
                </el-form-item>
...
export default {
  ...
  methods: {
    getData() {
      (this.loading = true),
        commonAPI("querylistsongs", this.formInline).then((res) => {
          this.loading = false;
    ...
    resetting() {
      this.formInline.listid = "";
      this.formInline.songname = "";
      this.getData();
    },
    ...
    //提交保存:添加或编辑用户
    submitAddOrEditUser() {
      commonAPI("addlistsongs", this.form).then((res) => {
    ...
    //删除用户:根据id删除用户
    async removeSingerByName(listid,songname) {
      ...
      this.listsong.listid=listid;
      this.listsong.songname=songname;
      const { data: res } = await commonAPI("deletelistsongs", this.listsong);
      // 刷新列表
      this.getData();
    },
  },
