      ...
      <template>
        <el-container>
          <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>歌曲库</el-breadcrumb-item>
          </el-breadcrumb>
          <el-header style="background-color: white">
            <el-form
              :inline="true"
              ...
              <el-table-column align="center" label="操作">
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    type="danger"
                    @click="removeSongById(scope.row.songid)"
                    >删除</el-button
                  >
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    getData() {
      (this.loading = true),
        commonAPI("querysongList1", this.formInline).then((res) => {
          this.loading = false;
    ...
    //提交保存:添加或编辑用户
    submitAddOrEditUser() {
      commonAPI("addsong", this.form).then((res) => {
        this.form = {};
        this.dialogFormVisible = false;
        this.getData();
      });
    ...
    async removeSongById(songid) {
      ...
      this.song.songid = songid;
      const { data: res } = await commonAPI("deletesong", this.song);
      // 刷新列表
      this.getData();
    },
  },
